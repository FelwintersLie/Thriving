...
class SchedulerTests(unittest.TestCase):
    ...
    def test_room_preference_tier_biases_choice(self):
        providers = [
            Provider(id="p1", name="Provider 1", disciplines={"Physical Therapy"}, templates=[ProviderTemplate("p1", 0, [TimeWindow(450, 1080)])]),
        ]
        patients = [Patient(id="i1", name="I1", availability={"2026-01-05": [TimeWindow(450, 1080)]})]
        rooms = [
            Room(id="r_high", name="High", capacity=4, allowed_disciplines={"Physical Therapy"}, preference_tier=3),
            Room(id="r_low", name="Low", capacity=4, allowed_disciplines={"Physical Therapy"}, preference_tier=0),
        ]
        ...
        result = ScheduleEngine().generate_schedule(
            date_key="2026-01-05",
            weekday=0,
            requests=[request],
            providers=providers,
            patients=patients,
            rooms=rooms,
            day_window=TimeWindow(450, 1080),
        )
        self.assertEqual(result["req_1"].room_id, "r_low")
