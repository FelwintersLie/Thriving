...
from app.room_rules import is_room_discipline_compatible, normalize_room_rules, room_is_available, room_preference_tier, room_rule_violations
...
class RoomRulesTests(unittest.TestCase):
    ...
    def test_room_overlap_detection_for_unavailable_and_available_only(self):
        ...
        self.assertIn("room_outside_available_only_weekly", violations)

    def test_room_discipline_compatibility_and_tier(self):
        rules = normalize_room_rules(
            {"rooms": {"Room 1": {"allowed_disciplines": ["Physical Therapy"], "preference_tier": 2}}},
            valid_rooms=["Room 1"],
            disciplines=["Physical Therapy", "Psychiatry"],
        )
        self.assertTrue(is_room_discipline_compatible(rules, "Room 1", "Physical Therapy"))
        self.assertFalse(is_room_discipline_compatible(rules, "Room 1", "Psychiatry"))
