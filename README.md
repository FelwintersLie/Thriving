...
### Room Compatibility, Room Tiers, and Provider Lunch Rules

- **Room Rules** now include per-room:
  - allowed disciplines (compatibility),
  - preference tier (`0` preferred, `1` semi-semi-last-resort, `2` semi-last-resort, `3` true last resort),
  - weekly/date availability restrictions.
- Auto-generation (IOP + EVAL) enforces room-discipline compatibility as a hard rule and uses room preference tier as a soft penalty to prefer lower-tier rooms when alternatives exist.
- Manual scheduling enforces room-discipline compatibility and shows a warning when selecting tier-3 rooms.
- Provider Profiles now support lunch settings:
  - `Enforce 30-min lunch break`
  - `Earliest lunch start` + `Latest lunch start`
- Lunch settings are treated as hard constraints during generation, and manual scheduling blocks appointments overlapping the enforced lunch range.
- Provider default availability behavior: when a provider has **no availability templates and no exceptions**, they are treated as available all day within the schedule day window.


### Profile Lifecycle + Date Validation Improvements

- The app auto-loads the most recently autosaved profile at startup, and also restores the last known source profile path when available.
- Corrupt/missing `data/last_profile.json` now falls back safely to a blank/default state instead of crashing startup.
- Closing the app with pending unsaved changes now shows a 3-option prompt:
  - **Save Profile and Close**
  - **Close without Saving Profile**
  - **Cancel**
- Date selectors now use dynamic day dropdowns that respect month/year and leap years (e.g., Feb has 29 days in leap years).
- Schedule headers now display dates as `Mon 12/25/26` (weekday + `MM/DD/YY`).
- Patient ID format is now `IOP1..IOP15` for IOP and `EV1..EV10` for evaluation patients; filters/grids/exports follow these IDs consistently.
- Generator runs now show a modal progress dialog with approximate percent, elapsed time, ETA, and cancel support.
- Solver effort limits were raised (100x scale-up) while preserving runtime caps for safety.
