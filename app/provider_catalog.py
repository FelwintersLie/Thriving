...
def _normalize_exceptions(raw_exceptions: Any) -> List[Dict[str, Any]]:
    ...
    for idx, item in enumerate(raw_exceptions):
        ...
        exceptions.append({"date": date_key, "kind": kind, "start_minute": start, "end_minute": end})
    return exceptions


def _normalize_lunch_settings(entry: Dict[str, Any]) -> Dict[str, Any]:
    enforce = bool(entry.get("enforce_lunch_break", False))
    earliest = int(entry.get("lunch_earliest_start_minute", 12 * 60))
    latest = int(entry.get("lunch_latest_start_minute", 13 * 60))
    if earliest % SLOT_MINUTES != 0 or latest % SLOT_MINUTES != 0:
        raise ProviderCatalogError("Lunch range must align to 15-minute increments")
    if latest < earliest:
        raise ProviderCatalogError("Lunch latest start must be >= earliest start")
    return {
        "enforce_lunch_break": enforce,
        "lunch_earliest_start_minute": earliest,
        "lunch_latest_start_minute": latest,
    }


def normalize_provider_entry(entry: Dict[str, Any], *, fallback_id: str, all_rooms: Iterable[str], disciplines: Iterable[str]) -> Dict[str, Any]:
    ...
    lunch = _normalize_lunch_settings(entry)
...
def normalize_provider_catalog(entries: Iterable[Dict[str, Any]], *, defaults: Iterable[str], all_rooms: Iterable[str], disciplines: Iterable[str]) -> List[Dict[str, Any]]:
...
def _contains(window: Dict[str, int], start: int, end: int) -> bool:
    return int(window.get("start_minute", 0)) <= start and end <= int(window.get("end_minute", 0))
...
def provider_is_available(profile: Dict[str, Any], *, date_key: str, weekday: int, start_minute: int, end_minute: int) -> bool:
    ...
    # Requested behavior: if no templates and no exceptions, treat as broadly available.
    if not templates and not exceptions:
        ...
        for tmpl in templates:
            ...
            for window in tmpl.get("windows", []):
                if _contains(window, start_minute, end_minute):
                    in_template = True
                    break
            if in_template:
                break
    ...
    for ex in exceptions:
        ...
        if not _overlaps(start_minute, end_minute, start, end):
            continue
    ...
    if bool(profile.get("enforce_lunch_break", False)):
        earliest = int(profile.get("lunch_earliest_start_minute", 12 * 60))
        latest = int(profile.get("lunch_latest_start_minute", 13 * 60))
        lunch_end_min = earliest + 30
        lunch_end_max = latest + 30
        if _overlaps(start_minute, end_minute, earliest, lunch_end_max):
            # overlapping lunch range blocks manual placement so generators can preserve a 30-min slot.
            return False
