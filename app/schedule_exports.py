...
def _patient_sort_key(value: Any) -> Tuple[int, int, str]:
    text = str(value)
...
def _format_header_date(date_key: str) -> str:
    try:
        d = date.fromisoformat(str(date_key))
        weekday = ["Mon", "Tue", "Wed", "Thur", "Fri", "Sat", "Sun"][d.weekday()]
        return f"{weekday} {d.strftime('%m/%d/%y')}"
    except Exception:
        return str(date_key)
def build_schedule_layout_model(
    ...
    *,
    ...
    if program_filter in {"IOP", "EVAL", "EV"}:
        normalized_filter = "EVAL" if program_filter == "EV" else program_filter
        filtered = [a for a in filtered if a.get("program_type", "IOP") == normalized_filter]
