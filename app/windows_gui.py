...
class SchedulerDesktopApp:
    ...
    def _build_layout(self) -> None:
        ...
        grid_wrap = ttk.Labelframe(main, text="Patient Schedule Grid", padding=8)
    ...
    def _build_form_panel(self, parent) -> None:
        ...
        shift = ttk.Labelframe(parent, text="Re-jigger Rule (beta)", padding=6)
        shift.pack(fill="x")
        ...
        ttk.Button(shift, text="Apply Re-jigger", command=lambda: self._safe_action(self.apply_rejigger)).grid(row=1, column=4, padx=6)
    ...
    def apply_rejigger(self) -> None:
        ...
        live_result = build_live_result_from_profile(profile)
        self.last_result = live_result
        self.status_var.set(f"Status: Re-jigger moved {moved} appointment(s)")
        self._render_patient_grid(profile, live_result)
        self._refresh_profile_preview()
