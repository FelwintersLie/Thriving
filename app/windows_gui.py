...
def _safe_tk_import():
    ...
    return tk, ttk, messagebox, scrolledtext, filedialog, simpledialog, colorchooser
...
class SchedulerDesktopApp:
    def __init__(self) -> None:
        tk, ttk, messagebox, scrolledtext, filedialog, simpledialog, colorchooser = _safe_tk_import()
        self.tk = tk
        self.ttk = ttk
    ...
    def _default_discipline_registry(self) -> List[Dict[str, Any]]:
        return [{"name": d, "show_in_iop": True, "show_in_eval": True, "display_color": DISCIPLINE_COLORS.get(d, "#ffb3c1")} for d in DISCIPLINES]
    ...
    def _normalize_discipline_registry(self, incoming: Any) -> List[Dict[str, Any]]:
        ...
        for row in incoming or []:
            ...
            if key in seen:
                continue
            seen.add(key)
            color_raw = str((row or {}).get("display_color", DISCIPLINE_COLORS.get(name, "#ffb3c1")) or "").strip()
            if not (len(color_raw) == 7 and color_raw.startswith("#")):
                color_raw = DISCIPLINE_COLORS.get(name, "#ffb3c1")
            rows.append({
                "name": name,
                "show_in_iop": bool((row or {}).get("show_in_iop", True)),
                "show_in_eval": bool((row or {}).get("show_in_eval", True)),
                "display_color": color_raw,
            })
        if not rows:
    ...
    def _build_disciplines_tab(self, parent) -> None:
        ...
        self.discipline_color_var = tk.StringVar(value="#ffb3c1")
        color_row = ttk.Frame(right)
        color_row.pack(fill="x", pady=(10, 0))
        ttk.Label(color_row, text="Color").pack(side="left")
        self.discipline_color_preview = tk.Label(color_row, text="      ", bg="#ffb3c1", relief="solid", bd=1)
        self.discipline_color_preview.pack(side="left", padx=6)
        ttk.Label(color_row, textvariable=self.discipline_color_var).pack(side="left", padx=(0, 6))
        ttk.Button(right, text="Choose Color…", command=lambda: self._safe_action(self.choose_selected_discipline_color)).pack(anchor="w", pady=(6, 0))
    ...
    def _refresh_discipline_list(self) -> None:
        ...
        for row in self.discipline_registry:
            ...
            if row.get("show_in_eval", True):
                flags.append("EVAL")
            self.discipline_list.insert(self.tk.END, f"{row['name']} ({'/'.join(flags) or 'hidden'}) [{row.get('display_color', '#ffb3c1')}]")
    ...
    def _on_discipline_select(self, _event=None) -> None:
        ...
        color = str(row.get("display_color", "#ffb3c1"))
        self.discipline_color_var.set(color)
        if hasattr(self, "discipline_color_preview"):
            self.discipline_color_preview.configure(bg=color)
    ...
    def add_discipline(self) -> None:
        ...
        self.discipline_registry.append({"name": name, "show_in_iop": True, "show_in_eval": True, "display_color": DISCIPLINE_COLORS.get(name, "#ffb3c1")})
        ...
        self._refresh_schedule_grid_if_loaded()
    ...
    def choose_selected_discipline_color(self) -> None:
        if not hasattr(self, "discipline_list") or not self.discipline_list.curselection():
            raise ValueError("Select a discipline first")
        idx = int(self.discipline_list.curselection()[0])
        current = str(self.discipline_registry[idx].get("display_color", "#ffb3c1"))
        picked, hex_color = self.colorchooser.askcolor(color=current, title="Choose discipline color")
        if not hex_color:
            return
        self.discipline_registry[idx]["display_color"] = str(hex_color)
        self.discipline_color_var.set(str(hex_color))
        if hasattr(self, "discipline_color_preview"):
            self.discipline_color_preview.configure(bg=str(hex_color))
        self._refresh_discipline_list()
        self._apply_discipline_registry_to_ui()
        if self.loaded_profile is not None:
            self.loaded_profile["discipline_registry"] = copy.deepcopy(self.discipline_registry)
    ...
    def _discipline_color_map(self) -> Dict[str, str]:
        mapping = dict(DISCIPLINE_COLORS)
        for row in self.discipline_registry:
            name = str(row.get("name", "")).strip()
            color = str(row.get("display_color", "")).strip()
            if name and len(color) == 7 and color.startswith("#"):
                mapping[name] = color
        return mapping

    def _refresh_schedule_grid_if_loaded(self) -> None:
