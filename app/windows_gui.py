...
IOP_PATIENT_ID_CHOICES = [f"IOP{i}" for i in range(1, 16)]
EVAL_PATIENT_ID_CHOICES = [f"EV{i}" for i in range(1, 11)]
PATIENT_ID_CHOICES = IOP_PATIENT_ID_CHOICES + EVAL_PATIENT_ID_CHOICES
DISCIPLINE_OPTIONS = sorted(DISCIPLINES)
ROOM_OPTIONS = sorted(PREDEFINED_ROOMS)
...
def provider_names_for_discipline(provider_profiles: List[Dict[str, Any]], discipline: str) -> List[str]:
    names = []
    for profile in provider_profiles:
        name = str(profile.get("provider_name", "")).strip()
        if not name:
            continue
        pdisc = str(profile.get("discipline", "")).strip()
        if discipline and pdisc and pdisc != discipline:
            continue
        names.append(name)
...
def patient_sort_key(value: Any) -> tuple:
    text = str(value)
    if text.startswith("IOP") and text[3:].isdigit():
        return (0, int(text[3:]), text)
    if text.startswith("EV") and text[2:].isdigit():
        return (1, int(text[2:]), text)
...
def format_date_short(date_key: str) -> str:
    d = date.fromisoformat(date_key)
    return f"{WEEKDAY_SHORT[d.weekday()]} {d.strftime('%m/%d/%y')}"
    ...
    return [
        {
            "id": name,
            "name": name,
            "disciplines": list(DISCIPLINE_OPTIONS),
            "templates": templates,
            "exceptions": [],
        ...
        records.append(
            {
                ...
                "lunch_earliest_start_minute": int(profile.get("lunch_earliest_start_minute", 12 * 60)),
        ...
        out.append(
            {
                ...
                "name": room,
                "capacity": 10,
                "allowed_disciplines": rr.get("allowed_disciplines", list(DISCIPLINE_OPTIONS)),
                "preference_tier": int(rr.get("preference_tier", room_preference_tier(rules, room))),
...
def build_provider_availability_preview_data(provider_profile: Dict[str, Any], day_start: int, day_end: int) -> Dict[int, List[Tuple[int, int]]]:
    ...
    exceptions = provider_profile.get("exceptions") or []
    if not templates and not exceptions:
        for weekday in range(5):
            preview[weekday].append((day_start, day_end))
        return preview
    for t in templates:
...
class SchedulerDesktopApp:
    def __init__(self) -> None:
        ...
        self.room_rules = load_room_rules(ROOM_OPTIONS, disciplines=DISCIPLINE_OPTIONS)
        ...
        save_room_rules(self.room_rules, valid_rooms=ROOM_OPTIONS, disciplines=DISCIPLINE_OPTIONS)
    ...
    def _build_layout(self) -> None:
        ...
        ttk.Combobox(view_controls, textvariable=self.program_filter_var, values=["Both", "IOP", "EV"], state="readonly", width=10).pack(side="left", padx=4)
    ...
    def _build_auto_generator_tab(self, parent) -> None:
        ...
        ttk.Combobox(frame, textvariable=self.auto_patients_var, values=[str(i) for i in range(1, 16)], state="readonly", width=8).grid(row=1, column=0, padx=2)

        ttk.Label(frame, text="Start Year").grid(row=0, column=1, sticky="w")
        self.auto_start_year_combo = ttk.Combobox(frame, textvariable=self.auto_start_year_var, values=[str(today.year + i) for i in range(0, 3)], state="readonly", width=8)
        self.auto_start_year_combo.grid(row=1, column=1, padx=2)
        ttk.Label(frame, text="Start Month").grid(row=0, column=2, sticky="w")
        self.auto_start_month_combo = ttk.Combobox(frame, textvariable=self.auto_start_month_var, values=[f"{m:02d}" for m in range(1, 13)], state="readonly", width=6)
        self.auto_start_month_combo.grid(row=1, column=2, padx=2)
        ttk.Label(frame, text="Start Day").grid(row=0, column=3, sticky="w")
        self.auto_start_day_combo = ttk.Combobox(frame, textvariable=self.auto_start_day_var, values=[f"{d:02d}" for d in range(1, 32)], state="readonly", width=6)
        self.auto_start_day_combo.grid(row=1, column=3, padx=2)
        self.auto_start_year_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.auto_start_year_var, self.auto_start_month_var, self.auto_start_day_var, self.auto_start_day_combo)))
        self.auto_start_month_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.auto_start_year_var, self.auto_start_month_var, self.auto_start_day_var, self.auto_start_day_combo)))
        ...
        room_values = ["Any compatible room"] + ROOM_OPTIONS
        ...
        self.auto_discipline_combo = ttk.Combobox(cond, textvariable=self.auto_discipline_var, values=DISCIPLINE_OPTIONS, state="readonly", width=20)
        self.auto_discipline_combo.grid(row=1, column=1, padx=2)
        self.auto_discipline_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(self._on_auto_requirement_filters_changed))
        ...
        self.auto_provider_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(self._on_auto_requirement_filters_changed))
        ...
        self._on_auto_requirement_filters_changed()
    ...
    def _build_eval_generator_tab(self, parent) -> None:
        ...
        self.eval_start_year_combo = ttk.Combobox(frame, textvariable=self.eval_start_year_var, values=self.year_choices if hasattr(self, "year_choices") else [str(datetime.utcnow().year)], state="readonly", width=8)
        self.eval_start_year_combo.grid(row=1, column=0, padx=2)
        ttk.Label(frame, text="Month").grid(row=0, column=1, sticky="w")
        self.eval_start_month_combo = ttk.Combobox(frame, textvariable=self.eval_start_month_var, values=[f"{m:02d}" for m in range(1, 13)], state="readonly", width=6)
        self.eval_start_month_combo.grid(row=1, column=1, padx=2)
        ttk.Label(frame, text="Day").grid(row=0, column=2, sticky="w")
        self.eval_start_day_combo = ttk.Combobox(frame, textvariable=self.eval_start_day_var, values=[f"{d:02d}" for d in range(1, 32)], state="readonly", width=6)
        self.eval_start_day_combo.grid(row=1, column=2, padx=2)
        self.eval_start_year_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.eval_start_year_var, self.eval_start_month_var, self.eval_start_day_var, self.eval_start_day_combo)))
        self.eval_start_month_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.eval_start_year_var, self.eval_start_month_var, self.eval_start_day_var, self.eval_start_day_combo)))
    ...
    def _build_provider_profiles_tab(self, parent) -> None:
        ...
        ttk.Combobox(right, textvariable=self.provider_profile_discipline_var, values=[""] + DISCIPLINE_OPTIONS, state="readonly", width=28).grid(row=row, column=1, sticky="w", padx=4)
        row += 1
        ...
        for idx, room in enumerate(ROOM_OPTIONS, start=1):
        ...
        self.provider_lunch_earliest_var = tk.StringVar(value="1200")
        self.provider_lunch_latest_var = tk.StringVar(value="1300")
        lunch = ttk.Labelframe(right, text="Lunch Break", padding=6)
        ...
        ttk.Combobox(lunch, textvariable=self.provider_lunch_earliest_var, values=time_choices, state="readonly", width=10).grid(row=2, column=0, padx=2, sticky="w")
        ttk.Combobox(lunch, textvariable=self.provider_lunch_latest_var, values=time_choices, state="readonly", width=10).grid(row=2, column=1, padx=2, sticky="w")
        row += 1
        ...
        today = datetime.utcnow().date()
        self.profile_exception_year_var = tk.StringVar(value=str(today.year))
        self.profile_exception_month_var = tk.StringVar(value=f"{today.month:02d}")
        self.profile_exception_day_var = tk.StringVar(value=f"{today.day:02d}")
        ...
        self.profile_exception_year_combo = ttk.Combobox(ex, textvariable=self.profile_exception_year_var, values=self.year_choices, state="readonly", width=6)
        self.profile_exception_year_combo.grid(row=0, column=1, padx=2)
        self.profile_exception_month_combo = ttk.Combobox(ex, textvariable=self.profile_exception_month_var, values=[f"{m:02d}" for m in range(1,13)], state="readonly", width=4)
        self.profile_exception_month_combo.grid(row=0, column=2, padx=2)
        self.profile_exception_day_combo = ttk.Combobox(ex, textvariable=self.profile_exception_day_var, values=[f"{d:02d}" for d in range(1,32)], state="readonly", width=4)
        self.profile_exception_day_combo.grid(row=0, column=3, padx=2)
        ttk.Combobox(ex, textvariable=self.profile_exception_start_var, values=time_choices, state="readonly", width=10).grid(row=0, column=4, padx=2)
        ttk.Combobox(ex, textvariable=self.profile_exception_end_var, values=time_choices, state="readonly", width=10).grid(row=0, column=5, padx=2)
        ttk.Button(ex, text="Add Exception", command=lambda: self._safe_action(self.add_provider_profile_exception)).grid(row=0, column=6, padx=4)
        ttk.Button(ex, text="Remove Exception", command=lambda: self._safe_action(self.remove_provider_profile_exception)).grid(row=0, column=7, padx=4)
        self.provider_exception_list = tk.Listbox(ex, height=5)
        self.provider_exception_list.grid(row=1, column=0, columnspan=8, sticky="ew", pady=(4, 0))
        self.profile_exception_year_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.profile_exception_year_var, self.profile_exception_month_var, self.profile_exception_day_var, self.profile_exception_day_combo)))
        self.profile_exception_month_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.profile_exception_year_var, self.profile_exception_month_var, self.profile_exception_day_var, self.profile_exception_day_combo)))
        row += 1

        preview_wrap = ttk.Frame(right)
        preview_wrap.grid(row=row, column=0, columnspan=2, sticky="nsew", pady=(8, 0))
        self.provider_profile_preview_canvas = tk.Canvas(preview_wrap, width=620, height=180, bg="white", highlightthickness=1, highlightbackground="#ced4da")
        self.provider_profile_preview_canvas.grid(row=0, column=0, sticky="nsew")
        pv_y = ttk.Scrollbar(preview_wrap, orient=tk.VERTICAL, command=self.provider_profile_preview_canvas.yview)
        pv_y.grid(row=0, column=1, sticky="ns")
        pv_x = ttk.Scrollbar(preview_wrap, orient=tk.HORIZONTAL, command=self.provider_profile_preview_canvas.xview)
        pv_x.grid(row=1, column=0, sticky="ew")
        self.provider_profile_preview_canvas.configure(yscrollcommand=pv_y.set, xscrollcommand=pv_x.set)
        self.provider_profile_preview_canvas.bind("<MouseWheel>", lambda e: self.provider_profile_preview_canvas.yview_scroll(int(-1 * (e.delta / 120)), "units"))
    ...
    def _build_room_rules_tab(self, parent) -> None:
        ...
        ttk.Label(right, text="Allowed disciplines").grid(row=1, column=0, sticky="w")
        self.room_allowed_disciplines_list = tk.Listbox(right, selectmode=tk.MULTIPLE, height=6, exportselection=False)
        self.room_allowed_disciplines_list.grid(row=1, column=1, sticky="ew", padx=4)
        for d in DISCIPLINES:
            self.room_allowed_disciplines_list.insert(tk.END, d)

        ttk.Label(right, text="Room preference tier").grid(row=2, column=0, sticky="w", pady=(6, 0))
        self.room_preference_tier_var = tk.StringVar(value="0")
        ttk.Combobox(right, textvariable=self.room_preference_tier_var, values=["0", "1", "2", "3"], state="readonly", width=8).grid(row=2, column=1, sticky="w", padx=4, pady=(6, 0))
        ...
        today = datetime.utcnow().date()
        self.room_rule_date_year_var = tk.StringVar(value=str(today.year))
        self.room_rule_date_month_var = tk.StringVar(value=f"{today.month:02d}")
        self.room_rule_date_day_var = tk.StringVar(value=f"{today.day:02d}")
        ...
        self.room_rule_date_year_combo = ttk.Combobox(dates, textvariable=self.room_rule_date_year_var, values=self.year_choices, state="readonly", width=6)
        self.room_rule_date_year_combo.grid(row=0, column=0, padx=2)
        self.room_rule_date_month_combo = ttk.Combobox(dates, textvariable=self.room_rule_date_month_var, values=[f"{m:02d}" for m in range(1,13)], state="readonly", width=4)
        self.room_rule_date_month_combo.grid(row=0, column=1, padx=2)
        self.room_rule_date_day_combo = ttk.Combobox(dates, textvariable=self.room_rule_date_day_var, values=[f"{d:02d}" for d in range(1,32)], state="readonly", width=4)
        self.room_rule_date_day_combo.grid(row=0, column=2, padx=2)
        ttk.Combobox(dates, textvariable=self.room_rule_date_start_var, values=time_choices, state="readonly", width=10).grid(row=0, column=3, padx=2)
        ttk.Combobox(dates, textvariable=self.room_rule_date_end_var, values=time_choices, state="readonly", width=10).grid(row=0, column=4, padx=2)
        ttk.Combobox(dates, textvariable=self.room_rule_date_type_var, values=["unavailable", "available-only"], state="readonly", width=14).grid(row=0, column=5, padx=2)
        ttk.Button(dates, text="Add Exception", command=lambda: self._safe_action(self.add_room_date_rule)).grid(row=0, column=6, padx=4)
        ttk.Button(dates, text="Remove Exception", command=lambda: self._safe_action(self.remove_room_date_rule)).grid(row=0, column=7, padx=4)
        self.room_date_rules_list = tk.Listbox(dates, height=6)
        self.room_date_rules_list.grid(row=1, column=0, columnspan=8, sticky="ew", pady=(4, 0))
        self.room_rule_date_year_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.room_rule_date_year_var, self.room_rule_date_month_var, self.room_rule_date_day_var, self.room_rule_date_day_combo)))
        self.room_rule_date_month_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.room_rule_date_year_var, self.room_rule_date_month_var, self.room_rule_date_day_var, self.room_rule_date_day_combo)))
    ...
    def _build_form_panel(self, parent) -> None:
        ...
        self.cal_year_combo = self.ttk.Combobox(cal, textvariable=self.cal_year_var, values=self.year_choices, state="readonly", width=8)
        self.cal_year_combo.grid(row=1, column=0, padx=2)
        self.ttk.Label(cal, text="Month").grid(row=0, column=1, sticky="w")
        self.cal_month_combo = self.ttk.Combobox(cal, textvariable=self.cal_month_var, values=self.month_choices, state="readonly", width=6)
        self.cal_month_combo.grid(row=1, column=1, padx=2)
        self.ttk.Label(cal, text="Day").grid(row=0, column=2, sticky="w")
        self.cal_day_combo = self.ttk.Combobox(cal, textvariable=self.cal_day_var, values=self.day_choices, state="readonly", width=6)
        self.cal_day_combo.grid(row=1, column=2, padx=2)
        self.cal_year_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.cal_year_var, self.cal_month_var, self.cal_day_var, self.cal_day_combo)))
        self.cal_month_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(lambda: self._sync_day_combo(self.cal_year_var, self.cal_month_var, self.cal_day_var, self.cal_day_combo)))
        ...
        self.appt_room_var = self.tk.StringVar(value=ROOM_OPTIONS[0])
        ...
        self.appt_room_combo = self.ttk.Combobox(appt, textvariable=self.appt_room_var, values=ROOM_OPTIONS, state="readonly", width=16)
        ...
        self.appt_discipline_combo = self.ttk.Combobox(appt, textvariable=self.appt_discipline_var, values=DISCIPLINE_OPTIONS, state="readonly", width=22)
        self.appt_discipline_combo.grid(row=1, column=6, padx=2)
        self.appt_discipline_combo.bind("<<ComboboxSelected>>", lambda _e: self._safe_action(self._on_manual_discipline_selected))
        ...
        self._sync_day_combo(self.cal_year_var, self.cal_month_var, self.cal_day_var, self.cal_day_combo)
        self._sync_day_combo(self.auto_start_year_var, self.auto_start_month_var, self.auto_start_day_var, self.auto_start_day_combo)
        self._sync_day_combo(self.eval_start_year_var, self.eval_start_month_var, self.eval_start_day_var, self.eval_start_day_combo)
        self._sync_day_combo(self.profile_exception_year_var, self.profile_exception_month_var, self.profile_exception_day_var, self.profile_exception_day_combo)
        self._sync_day_combo(self.room_rule_date_year_var, self.room_rule_date_month_var, self.room_rule_date_day_var, self.room_rule_date_day_combo)
    ...
    def _state_digest(self) -> str:
        payload = {
            ...
            "auto_conditions": self.auto_conditions,
        }
        try:
    ...
    def _sync_day_combo(self, year_var, month_var, day_var, day_combo) -> None:
        year = int(str(year_var.get()))
        month = int(str(month_var.get()))
        max_day = days_in_month(year, month)
        values = [f"{d:02d}" for d in range(1, max_day + 1)]
        day_combo["values"] = values
        if day_var.get() not in values:
            day_var.set(values[-1])
    ...
    def _refresh_provider_dropdowns(self) -> None:
        ...
        if hasattr(self, "appt_provider_var"):
            self._on_manual_provider_selected()
        if hasattr(self, "auto_provider_var"):
            self._on_auto_requirement_filters_changed()
    ...
    def refresh_provider_profile_list(self) -> None:
        ...
        for profile in sorted(self.provider_profiles, key=lambda p: str(p.get("provider_name", "")).lower()):
    ...
    def _load_provider_profile_into_editor(self, provider_id: str) -> None:
        ...
        self.provider_lunch_earliest_var.set(f"{int(profile.get('lunch_earliest_start_minute', 12*60))//60:02d}{int(profile.get('lunch_earliest_start_minute', 12*60))%60:02d}")
        self.provider_lunch_latest_var.set(f"{int(profile.get('lunch_latest_start_minute', 13*60))//60:02d}{int(profile.get('lunch_latest_start_minute', 13*60))%60:02d}")
    ...
    def _collect_editor_profile_payload(self, preserve_id: str | None = None) -> Dict[str, Any]:
        ...
        lunch_earliest = parse_time_input(self.provider_lunch_earliest_var.get())
        lunch_latest = parse_time_input(self.provider_lunch_latest_var.get())
        if lunch_latest < lunch_earliest:
            raise ValueError("Lunch latest start must be at or after earliest start")
        return {
            "provider_id": pid,
            "provider_name": name,
            ...
            "enforce_lunch_break": bool(self.provider_lunch_enforce_var.get()),
            "lunch_earliest_start_minute": lunch_earliest,
            "lunch_latest_start_minute": lunch_latest,
        }
    ...
    def add_provider_profile_exception(self) -> None:
        ...
        if not selected:
            raise ValueError("Select a provider profile first")
        date_key = parse_date_parts(self.profile_exception_year_var.get(), self.profile_exception_month_var.get(), self.profile_exception_day_var.get()).isoformat()
    ...
    def _render_provider_preview_canvas(self, canvas, profile: Dict[str, Any]) -> None:
        ...
        for slot in range(slots):
            ...
            for idx in range(len(weekdays)):
                x0 = left_w + idx * col_w
                x1 = x0 + col_w
                canvas.create_rectangle(x0, y0, x1, y1, fill="#ffdddd", outline="#f1c0c0")
        ...
        for wd, windows in preview.items():
            for start, end in windows:
                ...
                canvas.create_rectangle(x0, y0, x1, y1, fill="#c8f7c5", outline="#2e8b57", width=2)

        for ex in profile.get("exceptions", []):
            try:
                d = date.fromisoformat(str(ex.get("date") or ex.get("date_key") or ""))
            except Exception:
                continue
            wd = d.weekday()
            if wd < 0 or wd > 4:
                continue
            start = int(ex.get("start_minute") if "start_minute" in ex else ex.get("window", {}).get("start_minute", day_start))
            end = int(ex.get("end_minute") if "end_minute" in ex else ex.get("window", {}).get("end_minute", day_end))
            start_slot = max(0, (max(start, day_start) - day_start) // GRID_SLOT_MINUTES)
            end_slot = min(slots, (min(end, day_end) - day_start) // GRID_SLOT_MINUTES)
            if end_slot <= start_slot:
                continue
            x0 = left_w + wd * col_w + 6
            x1 = x0 + col_w - 12
            y0 = header_h + start_slot * row_h + 2
            y1 = header_h + end_slot * row_h - 2
            canvas.create_rectangle(x0, y0, x1, y1, fill="#ffd29d", outline="#ff8c00", width=2)
    ...
    def _room_rule_bucket(self, room_name: str) -> Dict[str, Any]:
        ...
        if room_name not in rooms:
            rooms[room_name] = {
                "allowed_disciplines": list(DISCIPLINE_OPTIONS),
                "preference_tier": int(room_preference_tier(self.room_rules, room_name)),
    ...
    def revert_room_rules_editor(self) -> None:
        ...
        self.room_allowed_disciplines_list.selection_clear(0, self.tk.END)
        allowed = bucket.get("allowed_disciplines") or list(DISCIPLINE_OPTIONS)
        for idx, name in enumerate(DISCIPLINES):
            if name in allowed:
                self.room_allowed_disciplines_list.selection_set(idx)
        self.room_preference_tier_var.set(str(int(bucket.get("preference_tier", room_preference_tier(self.room_rules, room_name)))))
    ...
    def add_room_date_rule(self) -> None:
        room_name = self._selected_room_name()
        date_key = parse_date_parts(self.room_rule_date_year_var.get(), self.room_rule_date_month_var.get(), self.room_rule_date_day_var.get()).isoformat()
    ...
    def save_room_rules(self) -> None:
        room_name = self._selected_room_name()
        bucket = self._room_rule_bucket(room_name)
        selected = [self.room_allowed_disciplines_list.get(i) for i in self.room_allowed_disciplines_list.curselection()]
        bucket["allowed_disciplines"] = selected or list(DISCIPLINE_OPTIONS)
        bucket["preference_tier"] = int(self.room_preference_tier_var.get() or "0")
    ...
    def _refresh_profile_preview(self) -> None:
        ...
        lines = [
            ...
            "Patient IDs available: IOP1-IOP15 and EV1-EV10",
    ...
    def _build_current_grid_layout(self, profile: Dict[str, Any], result: Dict[str, Any], visible_dates: List[str] | None = None) -> Dict[str, Any]:
        ...
        return build_schedule_layout_model(
            ...
            program_filter=("EVAL" if (self.program_filter_var.get()=="EV") else self.program_filter_var.get()) if hasattr(self, "program_filter_var") else "Both",
    ...
    def _allowed_rooms_for_provider(self, provider_profile: Dict[str, Any], discipline: str) -> List[str]:
        ...
        if "Any compatible room" in allowed:
            rooms = [r for r in ROOM_OPTIONS if is_room_discipline_compatible(self.room_rules, r, discipline)]
            return rooms or list(ROOM_OPTIONS)
        rooms = [r for r in allowed if r in ROOM_OPTIONS and is_room_discipline_compatible(self.room_rules, r, discipline)]
        return sorted(dict.fromkeys(rooms), key=str.lower)
    ...
    def _on_manual_provider_selected(self) -> None:
        ...
        if not profile:
            self.appt_room_var.set(ROOM_OPTIONS[0])
            return
        discipline = str(profile.get("discipline", "")).strip()
        if discipline:
    ...
    def _on_manual_discipline_selected(self) -> None:
        discipline = self.appt_discipline_var.get().strip()
        provider_name = self.appt_provider_var.get().strip()
        profile = self._provider_profile_by_name(provider_name)

        provider_choices = provider_names_for_discipline(self.provider_profiles, discipline)
        if hasattr(self, "appt_provider_combo"):
            self.appt_provider_combo["values"] = provider_choices

        if profile and str(profile.get("discipline", "")).strip() and str(profile.get("discipline", "")).strip() != discipline:
            profile = None

        if not profile:
            if provider_choices:
                if self.appt_provider_var.get() not in provider_choices:
                    self.appt_provider_var.set(provider_choices[0])
                profile = self._provider_profile_by_name(self.appt_provider_var.get().strip())
            else:
                return

        allowed_disciplines = [d for d in [str(profile.get("discipline", "")).strip()] if d]
        if allowed_disciplines and discipline not in allowed_disciplines:
            self.appt_discipline_var.set(allowed_disciplines[0])
            discipline = allowed_disciplines[0]

        rooms = self._allowed_rooms_for_provider(profile, discipline)
        if hasattr(self, "appt_room_combo"):
            self.appt_room_combo["values"] = rooms
        if self.appt_room_var.get() not in rooms and rooms:
            self.appt_room_var.set(rooms[0])

    def _on_auto_requirement_filters_changed(self) -> None:
        provider_name = self.auto_provider_var.get().strip() if hasattr(self, "auto_provider_var") else "Any provider"
        selected_discipline = self.auto_discipline_var.get().strip() if hasattr(self, "auto_discipline_var") else ""
        profile = self._provider_profile_by_name(provider_name) if provider_name and provider_name != "Any provider" else None

        if profile and str(profile.get("discipline", "")).strip():
            discipline_choices = [str(profile.get("discipline", "")).strip()]
            if selected_discipline not in discipline_choices:
                self.auto_discipline_var.set(discipline_choices[0])
            if hasattr(self, "auto_discipline_combo"):
                self.auto_discipline_combo["values"] = discipline_choices
            selected_discipline = self.auto_discipline_var.get().strip()
        else:
            if hasattr(self, "auto_discipline_combo"):
                self.auto_discipline_combo["values"] = DISCIPLINE_OPTIONS
            if selected_discipline not in DISCIPLINE_OPTIONS:
                self.auto_discipline_var.set(DISCIPLINE_OPTIONS[0])
            selected_discipline = self.auto_discipline_var.get().strip()

        provider_names = provider_names_for_discipline(self.provider_profiles, selected_discipline)
        primary_values = ["Any provider"] + provider_names
        optional_values = ["(none)"] + provider_names
        ...
        for var in [self.auto_provider_b_var, self.auto_provider_c_var]:
            if var.get() not in optional_values:
                var.set("(none)")

        compat_rooms = [r for r in ROOM_OPTIONS if is_room_discipline_compatible(self.room_rules, r, selected_discipline)]
        if profile:
            compat_rooms = [r for r in compat_rooms if r in self._allowed_rooms_for_provider(profile, selected_discipline)]
        room_values = ["Any compatible room"] + compat_rooms
        self.auto_room_combo["values"] = room_values
        if self.auto_room_var.get() not in room_values:
            self.auto_room_var.set("Any compatible room")
    ...
    def add_appointment(self) -> None:
        ...
        program_type = self.appt_program_var.get().strip() or ("EVAL" if patient_id.startswith("EV") else "IOP")
        ...
        if not is_room_discipline_compatible(self.room_rules, room_id, discipline):
            raise ValueError("Selected room is not compatible with this discipline")
        ...
        if room_preference_tier(self.room_rules, room_id) >= 3:
            self.messagebox.showwarning("Room tier warning", "Selected room is Tier 3 (true last resort).")
    ...
    def _selected_patient_scope_ids(self) -> List[str]:
        ...
        for pid in raw:
            if pid not in PATIENT_ID_CHOICES:
                raise ValueError(f"Patient ID '{pid}' must be in set IOP1-IOP15 or EV1-EV10")
        return raw
    ...
    def _runtime_limit_seconds_from_ui(self) -> int:
        effort = self.auto_effort_var.get().strip().lower() if hasattr(self, "auto_effort_var") else "high"
        mapping = {"standard": 30, "high": 60, "very high": 120, "maximum": 180}
        return mapping.get(effort, 60)

    def _solver_limits_from_ui(self) -> Dict[str, int]:
        effort = self.auto_solver_effort_var.get().strip().lower()
        mapping = {
            "standard": {"max_backtrack_states": 25000000, "max_candidates_per_request": 500000},
            "high": {"max_backtrack_states": 75000000, "max_candidates_per_request": 1000000},
            "very high": {"max_backtrack_states": 150000000, "max_candidates_per_request": 2000000},
            "maximum": {"max_backtrack_states": 300000000, "max_candidates_per_request": 3000000},
        }
    ...
    def _update_after_auto_generation(self, profile_template: Dict[str, Any], result: Dict[str, Any], default_program_type: str = "IOP") -> None:
        ...
        preserved_requests: List[Dict[str, Any]] = []
        if self.loaded_profile:
            for req in self.loaded_profile.get("requests", []):
                if str(req.get("program_type", "IOP")) != default_program_type:
                    preserved_requests.append(dict(req))
        profile = {
            **profile_template,
            "requests": preserved_requests + generated_requests,
        }
    ...
    def _run_generation_with_progress(self, title: str, worker):
        import threading
        import time
        ...
        dialog.grab_set()
        pct_var = self.tk.StringVar(value="Progress: 0%")
        elapsed_var = self.tk.StringVar(value="Elapsed: 00:00")
        eta_var = self.tk.StringVar(value="ETA: --:--")
        bar = self.ttk.Progressbar(dialog, mode="determinate", maximum=100, length=360)
        self.ttk.Label(dialog, textvariable=pct_var).pack(anchor="w", padx=10, pady=(10, 2))
        bar.pack(fill="x", padx=10)
        self.ttk.Label(dialog, textvariable=elapsed_var).pack(anchor="w", padx=10, pady=(6, 0))
        self.ttk.Label(dialog, textvariable=eta_var).pack(anchor="w", padx=10, pady=(2, 8))
        cancelled = threading.Event()
        self.ttk.Button(dialog, text="Cancel", command=cancelled.set).pack(pady=(0, 10))

        progress = {"percent": 0.0, "elapsed": 0.0, "eta": None}
        ...
        def progress_cb(payload):
            ...
            next_percent = max(0.0, min(100.0, (completed / total) * 100.0))
            progress["percent"] = max(progress["percent"], next_percent)
            progress["elapsed"] = float(payload.get("elapsed_seconds", progress["elapsed"]))
            progress["eta"] = payload.get("eta_seconds")

        def run_worker():
            try:
                outcome["result"] = worker(cancelled, progress_cb)
            except Exception as exc:
                outcome["error"] = exc
            finally:
                outcome["done"] = True
        ...
        def tick():
            elapsed_seconds = max(progress["elapsed"], time.monotonic() - started_at)
            elapsed = int(elapsed_seconds)
            pct_var.set(f"Progress: {progress['percent']:.0f}%")
            bar['value'] = progress['percent']
            elapsed_var.set(f"Elapsed: {elapsed // 60:02d}:{elapsed % 60:02d}")
            eta_value = progress["eta"]
            if eta_value is None and progress["percent"] > 0:
                eta_value = max(0.0, elapsed_seconds * (100.0 - progress["percent"]) / max(progress["percent"], 0.1))
            if eta_value is None:
                eta_var.set("ETA: --:--")
            else:
                eta = int(max(0, float(eta_value)))
                eta_var.set(f"ETA: {eta // 60:02d}:{eta % 60:02d}")
            if outcome["done"]:
                dialog.destroy()
                return
            dialog.after(150, tick)
    ...
    def generate_auto_schedule(self) -> None:
        ...
        result = self._run_generation_with_progress(
            "Generating IOP schedule",
            lambda cancel_event, progress_cb: generate_three_week_schedule(
                profile_template=profile_template,
                requirements=self.auto_conditions,
                solver_limits=self._solver_limits_from_ui(),
                max_runtime_seconds=self._runtime_limit_seconds_from_ui(),
    ...
    def generate_eval_schedule(self) -> None:
        ...
        result = self._run_generation_with_progress(
            "Generating EVAL schedule",
            lambda cancel_event, progress_cb: generate_eval_schedule(
                profile_template=profile_template,
                cohort_start=start,
    ...
    def generate_combined_schedule(self) -> None:
        ...
        def _combined_worker(cancel_event, progress_cb):
            iop = generate_three_week_schedule(
                ...
                max_runtime_seconds=self._runtime_limit_seconds_from_ui(),
                progress_callback=lambda p: progress_cb({**p, "completed": p.get("completed", 0), "total": max(1, p.get("total", 1) * 2)}),
                cancel_check=cancel_event.is_set,
            )
            ev = generate_eval_schedule(
                profile_template=profile_template,
                cohort_start=start,
                cohort_type=self.eval_cohort_var.get(),
                eval_patient_count=int(self.eval_patient_count_var.get()),
                group_duration_minutes=int(self.eval_group_duration_var.get()),
                group_start_time=parse_time_input(self.eval_group_start_var.get()),
                previous_assignments={**existing, **iop.get("assignments", {})},
                solver_limits=self._solver_limits_from_ui(),
                locked_request_ids=soft_locked_ids,
                max_runtime_seconds=self._runtime_limit_seconds_from_ui(),
                progress_callback=lambda p: progress_cb({**p, "completed": p.get("completed", 0) + p.get("total", 0), "total": max(1, p.get("total", 1) * 2)}),
                cancel_check=cancel_event.is_set,
            )
            return iop, ev

        iop_result, eval_result = self._run_generation_with_progress("Generating combined schedule", _combined_worker)
    ...
    def import_existing_schedule_json(self) -> None:
        ...
        for item in incoming:
            ...
            req.setdefault("program_type", "EVAL" if any(str(pid).startswith("EV") for pid in req.get("patient_ids", [])) else "IOP")
    ...
    def _show_close_prompt(self) -> str:
        ...
        self.ttk.Label(dialog, text="You have unsaved profile changes. What would you like to do?", wraplength=360).pack(padx=12, pady=12)
        btns = self.ttk.Frame(dialog)
        btns.pack(fill="x", padx=10, pady=(0, 10))
        self.ttk.Button(btns, text="Save Profile and Close", command=lambda: choice.set("save")).pack(fill="x", pady=2)
        self.ttk.Button(btns, text="Close without Saving Profile", command=lambda: choice.set("discard")).pack(fill="x", pady=2)
        self.ttk.Button(btns, text="Cancel", command=lambda: choice.set("cancel")).pack(fill="x", pady=2)

        def close_wait(*_):
            if choice.get() in {"save", "discard", "cancel"}:
                dialog.destroy()

        choice.trace_add("write", close_wait)
        self.root.wait_window(dialog)
        return choice.get()

    def _on_close_requested(self) -> None:
        if self.profile_dirty:
            action = self._show_close_prompt()
            if action == "cancel":
                return
            if action == "save":
                try:
