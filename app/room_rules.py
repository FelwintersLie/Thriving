...
def normalize_room_rules(raw: Dict[str, Any] | None, *, valid_rooms: Iterable[str], disciplines: Iterable[str] | None = None) -> Dict[str, Any]:
    ...
    for room in valid:
        ...
        if not isinstance(room_raw, dict):
            room_raw = {}
        allowed_disciplines = room_raw.get("allowed_disciplines")
        if isinstance(allowed_disciplines, list):
            normalized_disciplines = sorted({str(d).strip() for d in allowed_disciplines if str(d).strip() and (not discipline_set or str(d).strip() in discipline_set)})
            if not normalized_disciplines and discipline_set:
                normalized_disciplines = sorted(discipline_set)
        else:
            normalized_disciplines = sorted(discipline_set)

        tier = int(room_raw.get("preference_tier", DEFAULT_ROOM_TIER_BY_NAME.get(room, 0)))
        if tier < 0:
            tier = 0
        if tier > 3:
            tier = 3
        ...
        result["rooms"][room] = {
            "allowed_disciplines": normalized_disciplines,
...
def is_room_discipline_compatible(rules: Dict[str, Any], room_id: str, discipline: str) -> bool:
    ...
    if not isinstance(room, dict):
        return True
    allowed = room.get("allowed_disciplines") or []
    if not allowed:
        return True
    return str(discipline).strip() in {str(d).strip() for d in allowed}
...
def room_preference_tier(rules: Dict[str, Any], room_id: str) -> int:
    ...
    if not isinstance(room, dict):
        return 0
    try:
        return max(0, min(int(room.get("preference_tier", DEFAULT_ROOM_TIER_BY_NAME.get(room_id, 0))), 3))
...
def room_rule_violations(
    ...
    *,
    ...
    if available_only_windows and not any(_contains(w, start_minute, end_minute) for w in available_only_windows):
        violations.append("room_outside_available_only_weekly")
    ...
    if available_only_dates and not any(_contains(w, start_minute, end_minute) for w in available_only_dates):
        violations.append("room_outside_available_only_date")
