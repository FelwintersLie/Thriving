...
def _solve_multiday(
    *,
    ...
    total_dates = max(1, len([d for d in profile_template["planning_dates"] if requests_by_date.get(d)]))
    completed_dates = 0

    for date_key in profile_template["planning_dates"]:
        if cancel_check and cancel_check():
            raise AutoScheduleError("Generation cancelled by user")
        ...
        try:
            ...
            if progress_callback:
                ...
                rate = completed_dates / elapsed
                remaining = max(0.0, (total_dates - completed_dates) / rate) if rate > 0 else 0.0
                progress_callback({"phase": "daily-solve", "completed": completed_dates, "total": total_dates, "elapsed_seconds": elapsed, "eta_seconds": remaining})
...
def generate_eval_schedule(
    *,
    ...
    eval_ids = [f"EV{i}" for i in range(1, eval_patient_count + 1)]
