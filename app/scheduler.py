...
MAX_PATIENTS = 256
MAX_ROOMS = 64
MAX_CANDIDATES_PER_REQUEST = 500000
MAX_BACKTRACK_STATES = 25000000
MAX_CANDIDATES_PER_REQUEST_HARD_CAP = 5000000
MAX_BACKTRACK_STATES_HARD_CAP = 500000000
...
class Provider:
    ...
    lunch_earliest_start_minute: int = 12 * 60
    lunch_latest_start_minute: int = 13 * 60

    def is_available(self, date_key: str, weekday: int, start: int, end: int) -> bool:
        # If provider has no templates and no exceptions, treat as all-day available.
        if not self.templates and not self.exceptions:
            in_template = True
        else:
        ...
        if self.enforce_lunch_break:
            lunch_block_start = self.lunch_earliest_start_minute
            lunch_block_end = self.lunch_latest_start_minute + 30
            if not (end <= lunch_block_start or start >= lunch_block_end):
                return False
        return True
...
class ScheduleEngine:
    ...
    def generate_schedule(
        ...
        *,
        ...
        def request_priority(r: SessionRequest) -> Tuple[int, int, int, int, int]:
            ccount = len(candidates[r.id])
            window_span = (r.preferred_window.end_minute - r.preferred_window.start_minute) if r.preferred_window else (day_window.end_minute - day_window.start_minute)
            specificity = int(bool(r.provider_id or r.provider_ids)) + int(bool(r.room_id))
            return (ccount, window_span, -r.duration_minutes, -specificity, len(r.patient_ids))
        ...
        def score(solution: Dict[str, Assignment]) -> Tuple[int, int]:
            ...
            for req in requests:
                ...
                room_obj = room_by_id.get(new.room_id)
                if room_obj:
                    preference_penalty += max(0, int(room_obj.preference_tier))
        ...
        def backtrack(index: int, assigned: Dict[str, Assignment]) -> None:
            ...
            if cancel_check and cancel_check():
                raise UnschedulableError("Generation cancelled by user")
            if max_runtime_seconds and (time.monotonic() - start_time) > float(max_runtime_seconds):
                raise UnschedulableError("Time limit reached while scheduling. Narrow windows or increase time cap.")
            if progress_callback and states_visited % 500 == 0:
                progress_callback({"states_visited": states_visited, "state_limit": backtrack_limit, "requests_total": len(sorted_requests), "requests_placed": index})
